...
-- MAGIC
-- MAGIC **Your DLT Pipeline is ready!** Your pipeline was started using this notebook and is <a dbdemos-pipeline-id="dlt-loans" href="/#joblist/pipelines/d1ed0df7-d84e-41ed-9eea-ec003b89fa92">available here</a>.
-- MAGIC
-- MAGIC <!-- Collect usage data (view). Remove it to disable collection. View README for more details.  -->
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=data-engineering&org_id=3140274369437577&notebook=%2F01-DLT-Loan-pipeline-SQL&demo_name=dlt-loans&event=VIEW&path=%2F_dbdemos%2Fdata-engineering%2Fdlt-loans%2F01-DLT-Loan-pipeline-SQL&version=1">
...
-- %fs ls /Volumes/dev_bronze/dbdemos_dlt_loan/raw_data/raw_transactions
...
AS SELECT * FROM STREAM read_files('/Volumes/dev_bronze/dbdemos_dlt_loan/raw_data/raw_transactions/', format => 'json', inferColumnTypes => true)
...
AS SELECT * FROM STREAM delta.`/Volumes/dev_bronze/dbdemos_dlt_loan/raw_data/ref_accounting_treatment/`
...
AS SELECT * FROM STREAM read_files('/Volumes/dev_bronze/dbdemos_dlt_loan/raw_data/historical_loans', format => 'csv', inferColumnTypes => true)
...
-- MAGIC %md ## Next steps
-- MAGIC
-- MAGIC Your DLT pipeline is ready to be started. <a dbdemos-pipeline-id="dlt-loans" href="/#joblist/pipelines/d1ed0df7-d84e-41ed-9eea-ec003b89fa92">Click here to access the pipeline</a> created for you using this notebook.
-- MAGIC
...
-- MAGIC
-- MAGIC <a dbdemos-dashboard-id="dlt-expectations" href='/sql/dashboardsv3/01f0473132e31302b37b83e78022c167' target="_blank">Data Quality Dashboard example</a>